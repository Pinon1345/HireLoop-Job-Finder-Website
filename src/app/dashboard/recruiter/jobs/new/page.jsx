...
// GravityUI Icons (importing SVG icons using the React standard)
...
// TODO: Mock Active Recruiter Company & Plan Details

const mockCompanyDetails = {
    id: "comp_101",
    name: "HireLoop Tech",
    approved: true,
    plan: "Free", // Can be "Free" | "Growth" | "Enterprise"
    activeJobs: 2,
    jobLimit: 3, // Free limit is 3
};
...
export default function PostJobPage() {
    ...
    const isLimitReached = mockCompanyDetails.activeJobs >= mockCompanyDetails.jobLimit;
    const isEligibleToPost = mockCompanyDetails.approved && !isLimitReached;
    ...
    const handleSubmit = async (e) => {
        ...
        const jobData = {
            ...formData,
            companyId: mockCompanyDetails.id,
            status: "active",
            createdAt: new Date().toISOString(),
        };

        // Simulate API storage / saving state
        ...
        if (res.insertedId) {
            ...
            toast.error("Ahh! Job Posted Failed! Try Again Later.")
        }
        ...
        setTimeout(() => {
            ...
            setSuccessMsg("Job posted successfully and is now active!");
    ...
    return (
        <div className="min-h-screen bg-[#0F0F10] text-[#E4E4E7] flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8">
            ...
            <div className="w-full max-w-3xl bg-[#18181B] border border-[#27272A] rounded-xl shadow-lg overflow-hidden p-8 shadow-blue-600 mb-4">
                ...
                {/* Header / Style matching provided image */}
                ...
                <div className="mb-8 p-4 bg-[#202024] rounded-lg border border-[#2F2F33] flex justify-between items-center text-sm">
                    <div>
                        ...
                        <span className="font-semibold text-white">{mockCompanyDetails.name}</span>
                        <span className="ml-2 text-xs bg-[#2E2E33] px-2.5 py-0.5 rounded-full text-zinc-300">
                            {mockCompanyDetails.plan} Plan
                        </span>
                    </div>
                    <div className="text-right">
                        <span className="text-[#A1A1AA]">Active Usage: </span>
                        <span className={`${isLimitReached ? "text-red-400 font-bold" : "text-emerald-400"}`}>
                            {mockCompanyDetails.activeJobs} / {mockCompanyDetails.jobLimit} jobs
                        </span>
                    </div>
                </div>

                {!isEligibleToPost ? (
                    <div className="bg-red-950/40 border border-red-900 text-red-200 p-4 rounded-lg text-sm text-center">
                        {isLimitReached
                            ? "You've reached the maximum active job listings allowed on your current plan. Please upgrade to post more."
                            : "Your company verification status is pending approval. You'll be able to post once approved."
                        }
                    </div>
                ) : (
                    <Form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <Fieldset className="space-y-6">
                            ...
                            <Fieldset.Group className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div className="col-span-2 flex flex-col md:flex-row gap-5 items-end">
                                    ...
                                    {/* Remote Toggle Switch Styled manually to fit custom Styleguide */}
                        ...
                        {/* Action Buttons styled precisely to match the cancel / submit from your image */}
                ...
                {successMsg && (
                    <div className="mt-6 p-4 bg-emerald-950/40 border border-emerald-900 text-emerald-200 text-sm rounded-lg text-center">
                        {successMsg}
                    </div>
                )}
            </div>
